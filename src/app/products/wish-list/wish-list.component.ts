...
export class WishListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getwishlist().subscribe(
      (data:any)=>{
        this.wishlist=data.products     // if our wishlist have products then show the products
        if(this.wishlist.length==0){    // if our wish list is empty then show the empty wishlist picture
